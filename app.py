...
st.markdown("**Versión completa 2026** – The Odds API + Kelly Criterion")
...
# ====================== LIGAS ACTUALIZADAS ======================
leagues = {
    "Todas las ligas de fútbol (recomendado)": "soccer",
    "Premier League (Inglaterra)": "soccer_epl",
    "La Liga (España)": "soccer_spain_laliga",
    ...
    "Chile Campeonato": "soccer_chile_campeonato"
}

# ====================== SIDEBAR ======================
with st.sidebar:
    st.header("📥 Datos generales")
    n = st.slider("Número de selecciones", min_value=3, max_value=8, value=4)
    ...
    selected_league_name = st.selectbox("Liga / Torneo", list(leagues.keys()))
    selected_sport = leagues[selected_league_name]

    if st.button("🔄 Cargar cuotas en vivo"):
        if not THE_ODDS_API_KEY or THE_ODDS_API_KEY.strip() == "":
            ...
            with st.spinner("Cargando cuotas..."):
                ...
                if resp.status_code == 200:
                    ...
                    for event in data[:20]:
                        ...
                        for book in event["bookmakers"]:
                            for market in book.get("markets", []):
                                if market.get("key") == "h2h" and len(market.get("outcomes", [])) >= 3:
                        ...
                        matches.append({
                            ...
                            "Hora": event.get("commence_time", "")[:16].replace("T", " "),
                            "1 (Local)": round(best_home, 2),
                            "X (Empate)": round(best_draw, 2),
                            "2 (Visitante)": round(best_away, 2),
                    ...
                    if matches:
                        ...
                        st.warning("No se encontraron partidos con odds 1X2.")
                else:
...
if st.button("🚀 Calcular todo (Sistemas + Value + Kelly)", type="primary", use_container_width=True):
    ...
    with tab2:
        st.subheader("Comparación de sistemas")
    ...
    with tab3:
        ...
        st.markdown("### Distribución exacta para Betano")
    ...
    with tab4:
        st.subheader("Escenarios según aciertos")
    ...
    with tab5:
        st.subheader("Cuotas en vivo")
        if "live_matches" in st.session_state:
            st.dataframe(st.session_state["live_matches"], use_container_width=True, height=600)
            st.caption("Copia las cuotas y pégalas arriba.")
        else:
            st.info("Pulsa '🔄 Cargar cuotas en vivo' en la barra lateral.")
